...
def ProdolzhiPravo(x, y, precki, dir):
    if dir == "istok":
        if (x + 1, y) not in precki and x + 1 < 10:
            return x + 1, y, dir
    elif dir == "sever":
        if (x, y + 1) not in precki and y + 1 < 10:
            return x, y + 1, dir
    elif dir == "jug":
        if (x, y - 1) not in precki and y - 1 >= 0:
            return x, y - 1, dir
    elif dir == "zapad":
        if (x - 1, y) not in precki and x - 1 >= 0:
            return x - 1, y, dir
    return -1, -1, dir


def ProdolzhiNazad(x, y, precki, dir):
    if dir == "istok":
        if (x - 1, y) not in precki and x - 1 >= 0:
            return x - 1, y, "zapad"
    elif dir == "zapad":
...
class Pacman(Problem):
    def __init__(self, initial, precki, goal=None):
    ...
    def successor(self, state):
        ...
        new_xy = ProdolzhiPravo(px, py, self.precki, nasoka)
        if new_xy[0] != -1 and new_xy[1] != -1:
            succ["ProdolzhiPravo"] = (new_xy[0], new_xy[1], new_xy[2],
                                      tuple([s for s in food if s[0] != new_xy[0] or s[1] != new_xy[1]]))
        new_xy = ProdolzhiNazad(px, py, self.precki, nasoka)
        if new_xy[0] != -1 and new_xy[1] != -1:
            succ["ProdolzhiNazad"] = (new_xy[0], new_xy[1], new_xy[2],
                                      tuple([s for s in food if s[0] != new_xy[0] or s[1] != new_xy[1]]))
        new_xy = SvrtiLevo(px, py, self.precki, nasoka)
        if new_xy[0] != -1 and new_xy[1] != -1:
            succ["SvrtiLevo"] = (new_xy[0], new_xy[1], new_xy[2],
                                 tuple([s for s in food if s[0] != new_xy[0] or s[1] != new_xy[1]]))
        new_xy = SvrtiDesno(px, py, self.precki, nasoka)
        if new_xy[0] != -1 and new_xy[1] != -1:
            succ["SvrtiDesno"] = (new_xy[0], new_xy[1], new_xy[2],
                                  tuple([s for s in food if s[0] != new_xy[0] or s[1] != new_xy[1]]))
        return succ
    ...
    def h(self, node):
        x = node.state[0]
        y = node.state[1]
        foodcoord = list(node.state[3])
        for i in range(len(foodcords)):
            if abs(x - foodcords[i][0] + abs(y - foodcords[i][1])):
                max = abs(x - foodcords[0][0]) + abs(y - foodcords[0][1])
        return max
...
if __name__ == "__main__":
    px = int(input())
    py = int(input())
    nasoka = input()  # istok, zapad, sever, jug
    brhrana = int(input())
    nizaKoord = list()
    for i in range(0, brhrana):
        el = input()
        br = el.split(",")
        add = int(br[0]), int(br[1])
        nizaKoord.append(add)
    nizaKoord = tuple(nizaKoord)

    precki = ((0, 6), (0, 8), (0, 9), (1, 2), (1, 3), (1, 4), (1, 9), (2, 9),
              (3, 6), (3, 9), (4, 1), (4, 5), (4, 6), (4, 7), (5, 1), (5, 6),
              (6, 0), (6, 1), (6, 2), (6, 9), (8, 1), (8, 4), (8, 7), (8, 8),
              (9, 4), (9, 7), (9, 8))
    covek = Pacman((px, py, nasoka, nizaKoord), precki)
    result = astar_search(covek)
